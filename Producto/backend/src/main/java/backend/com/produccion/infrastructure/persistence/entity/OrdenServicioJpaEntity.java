...
public class OrdenServicioJpaEntity {
        ...
        @JoinColumn(name = "op_id", nullable = false, foreignKey = @ForeignKey(name = "fk_os_op"))
        private OrdenProduccionJpaEntity ordenProduccion;

        @ManyToOne(fetch = FetchType.LAZY)
        @JoinColumn(name = "proveedor_id", nullable = false, foreignKey = @ForeignKey(name = "fk_os_proveedor"))
