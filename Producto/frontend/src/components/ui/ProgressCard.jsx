...
const ProgressCard = ({ item, details, stages, type = 'op' }) => {
    ...
    return (
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden mb-4">
            ...
            <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex-1 min-w-[200px]">
                    <div className="flex items-center space-x-3 mb-1">
                        ...
                        {/* Status Badge */}
                        <div className={`flex items-center space-x-1 px-2 py-0.5 rounded-full ${isDelayed ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'
                            }`}>
                            <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isDelayed ? 'bg-red-500' : 'bg-green-500'
                                }`} />
                    ...
                    {isDelayed && etapaCritica && (
                        <div className="flex flex-col mt-0.5 ml-1">
                            <div className="flex items-center text-[9px] font-black text-red-500 uppercase tracking-widest">
                                <AlertCircle className="w-2.5 h-2.5 mr-1" />
                                Etapa crítica: {etapaCritica} ({diasAtraso} días atraso)
                            </div>
                            {motivo && (
                                <div className="text-[8px] font-bold text-red-400 uppercase tracking-tighter ml-3.5">
                                    {motivo}
                                </div>
                            )}
                        </div>
                ...
                <div className="flex-[2] w-full max-w-2xl px-2">
                    <div className="relative flex items-center justify-between h-8 bg-gray-100 rounded-full p-1 border border-gray-200/50 shadow-inner">
                            ...
                            return (
                                <button
                                    ...
                                    className={`
                                        ...
                                        ${status === 'current' ? (isDelayed ? 'bg-red-600 text-white shadow-xl scale-105' : 'bg-blue-600 text-white shadow-xl scale-105') : ''}
                                        ${status === 'pending' ? 'text-gray-400 hover:text-gray-500' : ''}
                ...
                <div className="flex items-center space-x-3">
                    <div className="text-right mr-4">
                        <div className="text-lg font-black text-gray-800">{progressValue}%</div>
                        <p className="text-[9px] text-gray-400 font-black uppercase">Avance Total</p>
                    </div>
                    <button
                        onClick={() => setIsExpanded(!isExpanded)}
                        className={`p-2 rounded-xl transition-colors ${isExpanded ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
                    >
            ...
            {/* Expanded Content */}
            {isExpanded && (
                <div className="px-5 pb-5 pt-2 bg-gray-50/50 border-t border-gray-100 animate-in slide-in-from-top-4 duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {/* Stage Details */}
                        <div className="col-span-2">
                            <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4">Detalle por Etapa</h5>
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`p-3 rounded-2xl border cursor-pointer transition-all ${selectedStage === stage.id
                                                ...
                                                : isThisStageDelayed ? 'bg-red-50 border-red-100 opacity-100' : 'bg-white border-gray-100 opacity-60 hover:opacity-100'
                                                }`}
                                        >
                                            <div className="flex items-center justify-between mb-2">
                                                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-widest ${status === 'completed' ? 'bg-green-100 text-green-700' :
                                                    status === 'current' ? (isThisStageDelayed ? 'bg-red-600 text-white' : 'bg-blue-100 text-blue-700') :
                                            ...
                                            {isThisStageDelayed && (
                                                <div className="mt-1 text-[8px] font-black text-red-500 uppercase">
                        ...
                        {/* Extra Info */}
                        <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm self-start">
                            ...
                            <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className="text-blue-600 hover:text-blue-800 flex items-center gap-1.5 p-2 bg-blue-50 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all hover:shadow-sm"
                                >
