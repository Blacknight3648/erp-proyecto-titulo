...
import { useModelosSearch } from '../../hooks/useModelosSearch';

export default function ModeloComboField({ value = '', onChange, onSelectModelo = undefined, placeholder = 'Buscar...', readOnly = false, className = '' }) {
    ...
    const { loading, search, createModelo } = useModelosSearch();
    const filtered = search(query);
    const exactMatch = filtered.some(m => m.nombreModelo.toUpperCase() === query.trim().toUpperCase());
    ...
    const select = (modelo) => {
        ...
        onChange(modelo.nombreModelo.toUpperCase());
        setIsOpen(false);
        setQuery('');
    };
    ...
    const handleCreate = async () => {
        ...
        if (!nombre) return;
        const creado = await createModelo(nombre);
        if (creado) onSelectModelo?.(creado);
                ...
                >
                    ...
                    <div className="max-h-52 overflow-y-auto">
                        ...
                        {!loading && filtered.map((m) => {
                            const isSelected = m.nombreModelo.toUpperCase() === value.toUpperCase();
                            return (
                                <button
                                    key={m.idModelo}
                                    type="button"
