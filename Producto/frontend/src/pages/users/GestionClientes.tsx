      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-zinc-200/60">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-xl font-semibold tracking-tight text-zinc-900">Gestión de Clientes</h1>
              <p className="text-xs text-zinc-500 font-medium mt-0.5">Maestro centralizado y control de cuentas operacionales</p>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse table-fixed min-w-[700px]">
                <thead>
                  <tr className="border-b border-zinc-100 bg-zinc-50/70 text-[10px] font-bold text-zinc-400 uppercase tracking-widest">
                    <th className="p-4 pl-6 w-[35%]">Cliente</th>
                ...
                <tbody className="divide-y divide-zinc-100/70">
                  {filteredClientes.map((c) => {
                    ...
                    const isActive = c.activo;
                    const name = c.razonSocial || "Sin razón social";
                    const initial = name.charAt(0).toUpperCase();
                    const email = c.correoCliente || c.contactos?.[0]?.emailContacto || "Sin correo corporativo";
                    const run = c.runCliente ? formatRUN(c.runCliente) : "—";
                    const siglaLabel = c.sigla || "—";
                    const giroLabel = c.giro?.descripcionGiro || "Sin giro asignado";
