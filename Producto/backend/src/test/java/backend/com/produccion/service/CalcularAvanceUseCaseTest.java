...
class CalcularAvanceUseCaseTest {
    ...
    private OrdenTrabajo ot(long id, int total, int producida, int merma,
            FaseProduccion fase, EstadoOT estado) {
        return new OrdenTrabajo(id, 1L, 1L, 10L, 1, TipoOT.INTERNA,
    ...
    void calcular_conAvanceCompleto_retornaSemaforoVerde() {
        when(otRepository.findByOrdenProduccionId(10L))
                .thenReturn(List.of(ot(1L,100, 100, 0, FaseProduccion.CORTE, EstadoOT.FINALIZADA)));
    ...
    void calcular_conAvanceParcialAlto_retornaSemaforoAmarillo() {
        when(otRepository.findByOrdenProduccionId(10L))
                .thenReturn(List.of(ot(1L,100, 70, 0, FaseProduccion.CORTE, EstadoOT.EN_PROCESO)));
    ...
    void calcular_conAvanceParcialBajo_retornaSemaforoNaranja() {
        when(otRepository.findByOrdenProduccionId(10L))
                .thenReturn(List.of(ot(1L,100, 30, 0, FaseProduccion.CORTE, EstadoOT.EN_PROCESO)));
    ...
    void calcular_sinAvance_retornaSemaforoRojo() {
        when(otRepository.findByOrdenProduccionId(10L))
                .thenReturn(List.of(ot(1L,100, 0, 0, FaseProduccion.CORTE, EstadoOT.PENDIENTE)));
    ...
    void calcular_consideraMermasEnElPorcentaje() {
        ...
        when(otRepository.findByOrdenProduccionId(10L))
                .thenReturn(List.of(ot(1L,100, 50, 10, FaseProduccion.CONFECCION, EstadoOT.EN_PROCESO)));
    ...
    void calcular_conMultiplesOTs_sumaTotalesCorrectamente() {
        when(otRepository.findByOrdenProduccionId(10L)).thenReturn(List.of(
                ot(1L,100, 50, 0, FaseProduccion.CORTE, EstadoOT.EN_PROCESO),
                ot(2L, 200, 100, 50, FaseProduccion.CONFECCION, EstadoOT.EN_PROCESO)));
    ...
    void calcular_mapeaCamposDeCadaOT() {
        when(otRepository.findByOrdenProduccionId(10L))
                .thenReturn(List.of(ot(5L, 80, 40, 0, FaseProduccion.BORDADO, EstadoOT.EN_PROCESO)));
