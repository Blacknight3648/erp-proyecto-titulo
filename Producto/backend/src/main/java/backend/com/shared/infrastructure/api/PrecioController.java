 ...
 *
 * <p>
 * Un artículo puede tener múltiples precios según el tipo
 * (COSTO, VENTA, MAYORISTA) y la moneda utilizada.
 * Cada precio referencia un {@code Articulo} y una {@code Moneda} existentes.
 * </p>
 *
 * <p>
 * Errores posibles:
 * </p>
 * <ul>
 * <li>{@code 400} – Validación fallida (valor negativo, campos obligatorios
 * faltantes)</li>
...
public class PrecioController {
     ...
     */
    @PostMapping
    public ResponseEntity<PrecioDTO> crear(@Valid @RequestBody PrecioDTO request) {
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<PrecioDTO> actualizar(
            @PathVariable Integer id,
            @Valid @RequestBody PrecioDTO request) {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<PrecioDTO> obtenerPorId(@PathVariable Integer id) {
    ...
    public ResponseEntity<List<PrecioDTO>> listarPorArticulo(@PathVariable Integer idArticulo) {
