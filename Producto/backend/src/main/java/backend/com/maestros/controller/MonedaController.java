 ...
 *
 * <p>Base URL: {@code /api/v3/maestros/monedas}</p>
 *
 * <p>Las monedas se asocian a los precios de los artículos.
 * El código ISO (ej: CLP, USD, EUR) debe ser único en el sistema.</p>
 *
...
public class MonedaController {
     ...
     */
    @PostMapping
    public ResponseEntity<MonedaResponseDTO> crear(@Valid @RequestBody MonedaRequestDTO request) {
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<MonedaResponseDTO> actualizar(
            @PathVariable Integer id,
            @Valid @RequestBody MonedaRequestDTO request) {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<MonedaResponseDTO> obtenerPorId(@PathVariable Integer id) {
     ...
     */
    @GetMapping
    public ResponseEntity<List<MonedaResponseDTO>> listarTodas() {
        return ResponseEntity.ok(monedaService.listarTodas());
    }

    /**
     * Elimina una moneda por su ID.
     * <p><strong>Advertencia:</strong> si existen precios asociados a esta moneda,
     * la eliminación será rechazada por la base de datos.</p>
     *
