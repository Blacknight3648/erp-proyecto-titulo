 ...
 *
 * <p>Base URL: {@code /api/v3/maestros/composiciones}</p>
 *
 * <p>La composición describe los materiales que componen una tela
 * (ej: 100% Algodón, 50% Poliéster / 50% Algodón).
 * El código de composición debe ser único. Las composiciones pueden filtrarse
 * por clasificación: {@code NATURAL}, {@code SINTETICO}, {@code MIXTO}.</p>
 *
...
public class ComposicionController {
     ...
     *
     * @param request datos de la composición (código, descripción, clasificación, uso típico)
     * @return composición creada con su ID, HTTP 201
     */
    @PostMapping
    public ResponseEntity<ComposicionResponseDTO> crear(@Valid @RequestBody ComposicionRequestDTO request) {
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<ComposicionResponseDTO> actualizar(
            @PathVariable Integer id,
            @Valid @RequestBody ComposicionRequestDTO request) {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<ComposicionResponseDTO> obtenerPorId(@PathVariable Integer id) {
     ...
     */
    @GetMapping
    public ResponseEntity<List<ComposicionResponseDTO>> listarTodas() {
    ...
    public ResponseEntity<List<ComposicionResponseDTO>> listarPorClasificacion(
