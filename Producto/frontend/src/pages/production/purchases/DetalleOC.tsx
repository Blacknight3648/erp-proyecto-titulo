...
export default function DetalleOC({
    ...
    const startEdit = (item) => { setEditingItemId(item.idOCItem); setPrecioEdit(String(item.precioUnitario ?? '')); };
    const cancelEdit = () => { setEditingItemId(null); setPrecioEdit(''); };
    const commitEdit = async (idOCItem) => {
        const valor = Number(precioEdit);
        if (!Number.isFinite(valor) || valor < 0) return;
        await onActualizarPrecio(oc.idOC, idOCItem, valor);
        cancelEdit();
    };
...
function TablaItems({ oc, editingItemId, precioEdit, setPrecioEdit, startEdit, cancelEdit, commitEdit, puedeEditarPrecio, formatCLP }) {
    return (
        <div>
            <div className="p-6 border-b border-slate-50">
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">
                    {puedeEditarPrecio
                        ? 'Puedes ajustar el precio unitario mientras la OC esté EMITIDA'
                        : 'Precios congelados (OC ya enviada)'}
                </p>
            </div>
            <table className="w-full text-left">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                    <tr className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                        ...
                        <th className="py-4 text-right pr-8">Subtotal</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                    {(oc.items || []).map(item => (
                        <tr key={item.idOCItem} className="hover:bg-indigo-50/30 transition-colors">
                            ...
                            <td className="py-5 text-xs font-black text-slate-700 text-center tabular-nums">{item.cantidadComprada}</td>
                            ...
                            <td className="pr-8 py-5 text-right text-xs font-black text-slate-800 tabular-nums">{formatCLP(item.subtotal)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
                                ...
                                <tr key={it.ocItemId} className={`border-t border-slate-100 ${excede ? 'bg-rose-50' : ''}`}>
                                    ...
                                    <td className="px-3 py-2 text-right">
                                        <input type="number" min={0} value={it.cantidadConforme}
                                            onChange={(e) => updateItem(idx, 'cantidadConforme', e.target.value)}
                                            disabled={!recibida}
                                    ...
                                    <td className="px-3 py-2 text-right">
                                        <input type="number" min={0} value={it.cantidadRechazada}
                                            onChange={(e) => updateItem(idx, 'cantidadRechazada', e.target.value)}
                                            disabled={!recibida}
