...
export default function HCSolicitudForm({
    ...
    return (
        ...
        >
            {/* Action Bar */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-card/40 backdrop-blur-3xl p-8 rounded-[3rem] border-2 border-border shadow-2xl shadow-brand-indigo/10">
                <div className="flex items-center gap-5">
                    <div className="bg-foreground p-4 rounded-3xl shadow-xl shadow-foreground/10">
                        <ShoppingCart className="w-8 h-8 text-background" />
                    </div>
                    <div>
                        <h2 className="text-3xl font-black text-foreground tracking-tight uppercase">Generar Hoja de Compra</h2>
                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mt-1">A partir de una Orden de Producción aprobada</p>
                    </div>
                </div>
                ...
                <div className="flex gap-4">
                    <Button
                        variant="ghost"
                        onClick={onCancel}
                        className="px-8 h-12 rounded-2xl font-black text-xs uppercase tracking-widest text-muted-foreground hover:bg-card hover:text-brand-indigo transition-all active:scale-95"
                    >
                        Cancelar
                    </Button>
                    <Button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    <Card className="rounded-[2.5rem] border-border bg-card/60 backdrop-blur-md shadow-2xl shadow-foreground/5 border overflow-visible">
                        <CardHeader className="p-10 pb-4 border-b border-border">
                            <div className="flex items-center gap-4">
                                <div className="bg-brand-indigo/10 p-2 rounded-xl">
                                    <Hash className="w-6 h-6 text-brand-indigo" />
                                </div>
                                <div>
                                    <CardTitle className="text-sm font-black text-foreground uppercase tracking-[0.1em]">Orden de Producción Origen</CardTitle>
                                    <CardDescription className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground mt-1">La OP debe tener una versión de Costeo vinculada</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="p-10 space-y-8 overflow-visible">
                            <div className="space-y-4 relative" ref={dropdownRef}>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3 px-1">
                                        <Hash className="w-4 h-4 text-brand-indigo" />
                                        <label className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em]">Escribir o Buscar OP disponible</label>
                                    </div>
                                    {loading && (
                                        <span className="text-[9px] text-brand-indigo font-black uppercase tracking-wider animate-pulse">Cargando datos...</span>
                                    )}
                                </div>
                                ...
                                <div className="relative">
                                    <Input
                                        ...
                                        type="text"
                                        className="h-14 bg-card/80 border-border rounded-2xl px-5 pr-12 font-black text-sm text-foreground shadow-sm transition-all focus:border-brand-indigo/50 focus:bg-card"
                                    ...
                                    <AnimatePresence>
                                        {isDropdownOpen && (
                                            <motion.div
                                                ...
                                                className="absolute z-50 w-full mt-2 bg-card/95 backdrop-blur-md border border-border rounded-2xl shadow-2xl overflow-hidden max-h-60 overflow-y-auto custom-scrollbar"
                                            >
                                                {filteredOPs.length > 0 ? (
                                                    filteredOPs.map(op => {
                                                        ...
                                                        return (
                                                            <div
                                                                ...
                                                                className={`px-5 py-3.5 hover:bg-muted flex items-center justify-between cursor-pointer border-b border-border transition-colors last:border-0 ${
                                                                    isSelected ? 'bg-brand-indigo/10 text-brand-indigo font-extrabold' : ''
                                                                }`}
                                                            >
                                                                <div className="flex items-center gap-3">
                                                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs ${
                                                                        isSelected ? 'bg-brand-indigo text-white shadow-sm' : 'bg-muted text-muted-foreground'
                                                                    }`}>
                                                                        #{op.idOP}
                                                                    </div>
                                                                    <div>
                                                    ...
                                                    <div className="py-6 text-center text-xs font-bold text-muted-foreground uppercase tracking-widest">
                            ...
                            {opIdValido && (
                                ...
                                >
                                    {selectedOP ? (
                                        <div className="rounded-2xl border border-border p-6 bg-muted/50 space-y-4 transition-all">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Vista Previa de la OP</h4>
                                                    <h3 className="text-base font-black text-foreground tracking-tight mt-1">
                                            ...
                                            <div className="space-y-2.5 pt-2 border-t border-border">
                                                ...
                                                <div className="flex items-center gap-2.5 text-xs">
                                                    ...
                                                    <span className={`font-bold uppercase tracking-wide text-[10px] ${tieneCosteo ? 'text-muted-foreground' : 'text-warning font-extrabold'}`}>
                                                        {tieneCosteo
                                                ...
                                                <div className="flex items-center gap-2.5 text-xs">
                                                    ...
                                                    <span className={`font-bold uppercase tracking-wide text-[10px] ${!yaTieneHC ? 'text-muted-foreground' : 'text-destructive font-extrabold'}`}>
                                                        {!yaTieneHC
                                            ...
                                            {canSubmit ? (
                                                <div className="bg-success/10 border border-success/20 rounded-xl p-3 flex items-center gap-2 text-success text-[10px] font-black uppercase tracking-wider">
                                                    <CheckCircle2 className="w-4 h-4 shrink-0 text-success" />
                                                    <span>OP válida y aprobada. Listo para generar la Hoja de Compra.</span>
                                                </div>
                                            ) : (
                                                <div className="bg-destructive/10 border border-destructive/20 rounded-xl p-3 flex items-center gap-2 text-destructive text-[10px] font-black uppercase tracking-wider">
                                        ...
                                        <div className="rounded-2xl border border-destructive/20 p-5 bg-destructive/5 flex items-center gap-3">
                                            <AlertTriangle className="w-5 h-5 text-destructive shrink-0" />
                                            <div>
                                                <p className="text-[10px] font-black text-destructive uppercase tracking-widest">OP no encontrada</p>
                                                <p className="text-[9px] font-bold text-destructive/80 uppercase tracking-wider mt-0.5">
                ...
                <div className="space-y-8">
                    <Card className="rounded-[2.5rem] bg-sidebar border-none shadow-2xl p-4 overflow-hidden relative">
                        <CardHeader className="p-8 pb-4 relative z-10">
                            <CardTitle className="text-[10px] font-black text-brand-indigo uppercase tracking-[0.25em] mb-4">¿Cómo funciona?</CardTitle>
                        </CardHeader>
                        <CardContent className="p-8 pt-0 space-y-6 relative z-10">
                            <div className="space-y-3 text-[10px] text-sidebar-foreground font-bold uppercase tracking-tight leading-relaxed">
                                ...
                                <p>3. La HC nace en estado <span className="text-warning">BORRADOR</span>.</p>
                                <p>4. Una vez revisada, debes <span className="text-success">APROBARLA</span> para poder generar OCs.</p>
                            </div>

                            <div className="pt-6 border-t border-sidebar-border flex items-start gap-4">
                                <AlertCircle className="w-5 h-5 text-warning shrink-0" />
                                <p className="text-[9px] text-sidebar-muted font-bold leading-relaxed uppercase tracking-tighter italic">
                    ...
                    <Card className="rounded-[2.5rem] bg-card/40 backdrop-blur-md border border-border p-8 space-y-3 shadow-xl shadow-foreground/5">
                        <div className="flex items-center gap-3">
                            <Info className="w-4 h-4 text-brand-indigo" />
                            <h4 className="text-[10px] font-black text-foreground uppercase tracking-widest">Validaciones</h4>
                        </div>
                        <ul className="text-[10px] text-muted-foreground font-bold uppercase tracking-tight leading-relaxed list-disc pl-5 space-y-1">
                            <li>OP existente</li>
